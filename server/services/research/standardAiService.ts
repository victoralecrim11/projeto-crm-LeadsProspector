...
import type { LeadSourceContext, ResolvedDesign } from '../../../src/site-builder/contracts/research.js';
...
type Dependencies = { audit?: typeof auditCurrentSite; discoverModels?: typeof discoverModels; requestBlueprint?: typeof requestBlueprint };
...
export async function generateStandardAiSite(source: LeadSourceContext, selection: ModelSelection = { mode: 'auto' }, overrides?: { primary: string; accent: string }, credentials: Credentials = {}, dependencies: Dependencies = {}) {
  ...
  const fallback = (reason: GenerationMetadata['fallbackReason']) => ({
    blueprint: blueprintFromDesign(design), design, contract,
    warnings: ['A IA não estava disponível. O site foi criado usando o fallback determinístico do Design System.'],
    generation: { provider: 'standard', model: 'researched-family', modelId: design.specification.family.id, generatedAt: design.resolution.resolvedAt, blueprintVersion: 2, mode: 'standard-fallback' as const, fallbackUsed: true, fallbackReason: reason, designFamily: design.specification.family.id, guidance: reactToolkitProfile },
  });
  try {
    ...
    const raw = await (dependencies.requestBlueprint ?? requestBlueprint)(model, buildStandardAiPrompt(source, design, contract), credentials);
    const blueprint = preserveDesign(raw, design);
    return { blueprint, design, contract, warnings: [...catalog.warnings, ...blueprint.warnings], generation: { provider: model.provider, model: model.model, modelId: model.id, generatedAt: new Date().toISOString(), blueprintVersion: 2, mode: 'standard-ai' as const, fallbackUsed: false, designFamily: design.specification.family.id, guidance: reactToolkitProfile } };
  } catch (error) {
    if (error instanceof SiteAiError && (error.status === 503 || error.status === 429)) return fallback(error.status === 429 ? 'rate-limit' : 'provider-unavailable');
    throw error;
  }
}