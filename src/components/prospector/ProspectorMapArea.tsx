...
import "./ProspectorMapArea.css";
import { Info, X } from "lucide-react";
import { Lead } from "../../types";
import { isValidCoordinate } from "../../utils/coordinates";
import { openLeadBusinessSearch, openLeadLocation, openOpenStreetMapLocation } from "../../utils/openGoogleMaps";
...
function button(label: string, action: () => void, css: string) {
  ...
  el.type = "button"; el.className = "prospector-popup-action"; el.textContent = label; el.style.cssText = css; el.addEventListener("click", action);
  return el;
}

function popupContent(lead: Lead, onAdd: () => void, onDetails: () => void) {
  const root = document.createElement("div");
  root.className = "prospector-lead-popup";
  ...
  const actions = document.createElement("div"); actions.className = "prospector-popup-actions"; actions.style.cssText = "display:flex;flex-direction:column;gap:5px;padding-top:7px;border-top:1px solid #e2e8f0";
  ...
  const detailsButton = button("◎ Ver detalhes", onDetails, "padding:6px 8px;font-size:11px;border:1px solid #cbd5e1;border-radius:8px;cursor:pointer;background:linear-gradient(180deg,#ffffff,#e2e8f0);color:#1e293b;font-weight:700;box-shadow:0 1px 3px #0f172a24");
  detailsButton.classList.add("prospector-details-action");
  detailsButton.title = "Abrir todos os dados disponíveis desta empresa";
  actions.append(detailsButton);
  if (lead.name.trim()) {
    const businessSearch = button("🔎 Buscar empresa no Google Maps", () => openLeadBusinessSearch(lead), "padding:6px 8px;font-size:11px;font-weight:700;border:1px solid #a5b4fc;border-radius:8px;cursor:pointer;background:#eef2ff;color:#4338ca");
    businessSearch.title = "Busca aproximada por nome, categoria e localização. Confirme os dados antes de adicionar ao CRM.";
    actions.append(businessSearch);
  }
  actions.append(button("📍 Coordenada exata no Google Maps", () => openLeadLocation(lead), "padding:6px 8px;font-size:11px;border:1px solid #bae6fd;border-radius:8px;cursor:pointer;background:#e0f2fe;color:#0369a1"));
  ...
  if (lead.osmType && lead.osmId && isValidCoordinate(lead.geoLat, lead.geoLng)) {
    ...
    osm.className = "prospector-popup-action";
...
export const ProspectorMapArea: React.FC<Props> = (props) => {
  ...
  return <div className="prospector-map-shell xl:col-span-8 glass-panel p-1.5 sm:p-2 rounded-2xl border border-white/10 relative flex flex-col">
    <div className={`absolute top-4 left-4 right-4 z-[400] flex-col items-start gap-2 pointer-events-none ${props.activeMarkerLead ? "hidden sm:flex" : "flex"}`}>
    ...
    <div ref={elementRef} className="prospector-map-canvas w-full flex-1 rounded-xl overflow-hidden relative bg-slate-900 z-10" />
    <div className="flex items-center justify-between px-2 sm:px-3 py-1.5 sm:py-2 text-[10px] sm:text-[11px] leading-tight text-slate-400 border-t border-white/10 mt-1.5 sm:mt-2"><span>OpenStreetMap / Overpass • clique em um marcador para ver a origem</span></div>
  </div>;
};
